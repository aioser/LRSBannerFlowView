...
#import "LRSBannerPagedFlowView.h"

@interface LRSBannerPagedFlowView ()
...
@implementation LRSBannerPagedFlowView
...
- (void)initialize {
    ...
    subviewClassName = @"PGItemSubviewProtocol";
...
- (void)startTimer {
    ...
    if (self.orginPageCount > 1 && self.isOpenAutoScroll && self.isCarousel) {
        ...
        self.timer = [NSTimer timerWithTimeInterval:self.autoTime repeats:true block:^(NSTimer * _Nonnull timer) {
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                UIView<LRSBannerPageFlowCellProtocol> *cell = [_cells objectAtIndex:i];
                if (!cell || [cell isKindOfClass:[NSNull class]]) {
                    return;
                }
                ...
                if (self.itemScale != 0) {
                    CGRect originPageFrame = CGRectMake(_pageSize.width * i, 0, _pageSize.width, _pageSize.height);
                    double inset = _pageSize.width * (1 - self.itemScale) * 0.5;
                    if (delta < _pageSize.width) {
                        inset = inset * delta / _pageSize.width;
                    }
                    cell.frame = UIEdgeInsetsInsetRect(originPageFrame, UIEdgeInsetsMake(inset, inset, inset, inset));
                } else {
                    ...
                    if (delta < _pageSize.width) {
                        ...
                        cell.layer.transform = CATransform3DMakeScale((_pageSize.height-topBottomInset*2)/_pageSize.height,(_pageSize.height-topBottomInset*2)/_pageSize.height, 1.0);
                        ...
                        cell.layer.transform = CATransform3DMakeScale((_pageSize.height-self.topBottomMargin*2)/_pageSize.height,(_pageSize.height-self.topBottomMargin*2)/_pageSize.height, 1.0);
...
- (void)setPageAtIndex:(NSInteger)pageIndex {
    ...
    if ((NSObject *)cell == [NSNull null]) {
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(flowView:cellForPageAtIndex:)]) {
...
- (void)reloadData
{
    ...
    if (_needsReload) {
        ...
        //重置pageWidth
        _pageSize = CGSizeMake(self.bounds.size.width - 4 * self.leftRightMargin,(self.bounds.size.width - 4 * self.leftRightMargin) * 1 / 2);
